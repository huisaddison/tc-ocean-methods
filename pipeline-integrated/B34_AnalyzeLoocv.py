...
ymat = np.zeros((1, 1, len(y)))

for depth_idx in range(1):
...
#inv_var = 1 / LOOCV[:,:,2,:] ** 2
#norm_inv_var = inv_var / inv_var.sum(axis=2, keepdims=True)
#loocv_estimates = (norm_inv_var * (
...
#error_estimates = LOOCV[:,:,0,:].sum(axis=2)
...
#plt.axvline(np.median(LAMB[lhat]), color='black')
...
# This "technically incorrect" mean and spread estimation method suggest 
# that the leave-one-out errors are well-behaved, in the sense that they 
# do not vary much between trials (which is to be expected, because each 
# "dataset" only differs by one point, so we should expect them to be
# highly correlated.
